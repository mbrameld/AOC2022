...
class Monkey:
    ...
    def inspectAndThrow(self, monkies: dict[int, Monkey]):
...
for roundNum in range(1, 21):
    for monkey in parsedMonkies:
        monkey.inspectAndThrow(parsedMonkies)
...
print(reduce(mul, list(
    reversed(sorted([m.countItemsInspected() for m in parsedMonkies])))[:2]))
